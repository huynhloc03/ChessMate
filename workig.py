...
def render_board():
    ...
    # Font for the labels (a-h and 1-8) inside the tiles
    font = ImageFont.truetype(font_path, font_size)


    in_checkmate = board.is_checkmate()
    if in_checkmate and not board.turn:  # Check if it's the player's turn and they are in checkmate
        player_king_square = board.king(chess.WHITE if player_is_white else chess.BLACK)
